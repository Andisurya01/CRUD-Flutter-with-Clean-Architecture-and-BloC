...
Future<void> setupDependencies() async {
  ...
  getIt.registerSingleton<RemoteProductDataSource>(
      RemoteProductDataSource(getIt()));
  getIt.registerSingleton<RemoteHighlightDataSource>(
      RemoteHighlightDataSource(getIt()));
  ...
  getIt
      .registerSingleton<HighlightRepository>(HighlightRepositoryImpl(getIt()));
  ...
  getIt.registerSingleton<AddShoesUseCase>(AddShoesUseCase(getIt()));
  getIt.registerSingleton<UpdateShoeUseCase>(UpdateShoeUseCase(getIt()));
  
  // Use Case - Auth
  getIt.registerSingleton<LoginUseCase>(LoginUseCase(getIt()));

  // Use Case - Highlight
  getIt.registerSingleton<GetAllHighlightUseCase>(
      GetAllHighlightUseCase(getIt()));
  getIt.registerSingleton<AddHighlightUseCase>(AddHighlightUseCase(getIt()));
  getIt.registerSingleton<DeleteHighlightUseCase>(
      DeleteHighlightUseCase(getIt()));
  getIt.registerSingleton<GetNonHighlightUseCase>(
      GetNonHighlightUseCase(getIt()));

  // Bloc
  getIt.registerFactory<RemoteProductBloc>(
      () => RemoteProductBloc(getIt(), getIt(), getIt(), getIt()));
  getIt.registerFactory<RemoteHighlightBloc>(
      () => RemoteHighlightBloc(getIt(), getIt(), getIt(), getIt()));
