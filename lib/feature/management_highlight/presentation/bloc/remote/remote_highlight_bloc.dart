...
class RemoteHighlightBloc
    extends Bloc<RemoteHighlightEvent, RemoteHighlightState> {
  ...
  RemoteHighlightBloc(this.getHighlights, this.deleteHighlight,
      this.addHighlight, this.getNonHighlights)
      : super(const RemoteHighlightLoading()) {
  ...
  void onGetHighlights(
      GetHighlights event, Emitter<RemoteHighlightState> emit) async {
    emit(const RemoteHighlightLoading());
    final result = await getHighlights.call(
  ...
  void onGetNonHighlights(
      GetNonHighlights event, Emitter<RemoteHighlightState> emit) async {
    emit(const RemoteHighlightLoading());
    final result = await getNonHighlights.call(
        params: ParamsNonHighlight(limit: event.limit, offset: event.offset));
    if (result is DataSuccess) {
      if (result.data != null && result.data!.isNotEmpty) {
        emit(RemoteHighlightDone(result.data!));
      } else {
  ...
  void onDeleteHighlightById(
      DeleteHighlightById event, Emitter<RemoteHighlightState> emit) async {
    emit(const RemoteHighlightLoading());
  ...
  void onAddHighlight(
      AddHighlight event, Emitter<RemoteHighlightState> emit) async {
    emit(const RemoteHighlightLoading());
