...
class ProductListChecklist extends StatefulWidget {
  final List<HighlightEntity> highlights;

  const ProductListChecklist({required this.highlights, super.key});

  @override
  State<ProductListChecklist> createState() => _ProductListChecklistState();
}

class _ProductListChecklistState extends State<ProductListChecklist> {
  ...
  void _onDeleteHighlight(HighlightEntity highlight) async {
    ...
    await _waitForBlocOperation(bloc, AddHighlight(id: highlight.id));

    // Setelah AddHighlight selesai, dapatkan highlights yang tidak terpilih
    await _waitForBlocOperation(bloc, GetNonHighlights(limit: 100, offset: 0));
